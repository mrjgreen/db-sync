...
class FullSyncTest extends TestAbstract
{
    ...
    public function testADryRunDoesNothing()
    {
        ...
        $sync->dryRun(true);
        $result = $sync->sync($this->source, $this->target, $config);
        $this->assertNotEquals(0, $result->getRowsTransferred());
        $this->assertEquals(0, $result->getRowsAffected());
        ...
        $result = $sync->sync($this->source, $this->target, $config);
        $this->assertNotEquals(0, $result->getRowsTransferred());
        $this->assertNotEquals(0, $result->getRowsAffected());
    }

    /**
     * @dataProvider providerHashStrategy
     */
    public function testItCorrectlySyncsFullData(\DbSync\Hash\HashInterface $hash)
    {
        $this->createTestDatabases();

        $this->doSync($hash);

        $this->compareTableReal();
    }

    /**
     * @dataProvider providerHashStrategy
     */
    public function testItCorrectlySyncsAlreadyMatchingData(\DbSync\Hash\HashInterface $hash)
    {
     ...
     * @dataProvider providerHashStrategy
     */
    public function testItCorrectlySyncsDataWithDeletionsAtSource(\DbSync\Hash\HashInterface $hash)
    {
        $this->createTestDatabases(true, true);

        $result = $this->doSync($hash);
    ...
    public function testItCorrectlySyncsPartialData(\DbSync\Hash\HashInterface $hash)
    {
        ...
        $this->doSync($hash, $cols);

        $this->compareTableReal('*', false);

        $this->compareTableReal(implode(',', $cols), true);
    }

    /**
     * @dataProvider providerHashStrategy
     */
    public function testItCorrectlySyncsWithAWhereClause(\DbSync\Hash\HashInterface $hash)
    {
        $this->createTestDatabases();
    ...
    {
        $sourceData = $this->connection->query("SELECT $cols FROM $this->source $whereClause")->fetchAll();

        $destData = $this->connection->query("SELECT $cols FROM $this->target")->fetchAll();
  ...
  PRIMARY KEY (`customerNumber`,`customerName`)
    ...
    private function populateTestTable($table)
    {
        ...
        // The first 4 rows have the same customer number but a different name - this will test that it syncs multi key tables with a shared value in the first key column
        $this->connection->query("insert  into $table (`customerNumber`,`customerName`,`contactLastName`,`contactFirstName`,`return`,`addressLine1`,`addressLine2`,`city`,`state`,`postalCode`,`country`,`salesRepEmployeeNumber`,`creditLimit`) values (103,'Atelier graphique','Schmitt','Carine ','40.32.2555','54, rue Royale',NULL,'Nantes',NULL,'44000','France',1370,21000),
(112,'Signal Gift Stores','King','Jean','7025551838','8489 Strong St.',NULL,'Las Vegas','NV','83030','USA',1166,71800),
(112,'Australian Collectors, Co.','Ferguson','Peter','03 9520 4555','636 St Kilda Road','Level 3','Melbourne','Victoria','3004','Australia',1611,117300),
(112,'La Rochelle Gifts','Labrune','Janine ','40.67.8555','67, rue des Cinquante Otages',NULL,'Nantes',NULL,'44000','France',1370,118200),
(112,'Baane Mini Imports','Bergulfsen','Jonas ','07-98 9555','Erling Skakkes gate 78',NULL,'Stavern',NULL,'4110','Norway',1504,81700),
