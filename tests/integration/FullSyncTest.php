...
class FullSyncTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testItCorrectlySyncsFullData(\DbSync\Hash\HashInterface $hash)
    {
        ...
        $this->compareTableReal(self::DATABASE . '.customers1', self::DATABASE . '.customers2');
    }
    ...
    public function testItCorrectlySyncsPartialData(\DbSync\Hash\HashInterface $hash)
    {
        ...
        $this->compareTableReal(self::DATABASE . '.customers1', self::DATABASE . '.customers2',  '*', false);

        $this->compareTableReal(self::DATABASE . '.customers1', self::DATABASE . '.customers2', implode(',', $cols), true);
    }

    private function doSync(\DbSync\Hash\HashInterface $hash, $cols = array())
    {
        $sync = new \DbSync\DbSync(false, $hash);
    ...
    {
        $sourceData = $this->connection->query("SELECT $cols FROM $source")->fetchAll();

        $destData = $this->connection->query("SELECT $cols FROM $destination")->fetchAll();
