...
class FullSyncTest extends PHPUnit_Framework_TestCase
{
    ...
    private function doSync($cols = array())
    {
        $sync = new \DbSync\DbSync();

        $connection = (new \Database\Connectors\ConnectionFactory())->make(include __DIR__ . '/config.php');

        $source = new \DbSync\Table($connection, self::DATABASE, 'customers1');
        $target = new \DbSync\Table($connection, self::DATABASE, 'customers2');
    ...
    private function createTestDatabases($populateBoth = false)
    {
        ...
        $this->utilityDb->query("DROP TABLE IF EXISTS $dbName.customers1");
        $this->utilityDb->query("DROP TABLE IF EXISTS $dbName.customers2");
    ...
    {
        $this->utilityDb->query("CREATE TABLE $table (
